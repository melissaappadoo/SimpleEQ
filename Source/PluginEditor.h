...
struct FFTDataGenerator
{
    ...
    int getNumAvailabkeFFTDataBlocks() const { return fftDataFifo.getNumAvailableForReading(); }
    ...
    void generatePath(const std::vector<float>& renderData, juce::Rectangle<float> fftBounds, int fftSize, float binWidth, float negativeInfinity)
    {
        ...
        for (int binNum = 1; binNum < numBins; binNum += pathResolution)
        {
            ...
            if (!std::isnan(y) && !std::isinf(y))
            {
                ...
                auto normalizedBinX = juce::mapFromLog10(binFreq, 20.f, 2000.f);
...
struct RotartSliderWithLabels : juce::Slider
{
    RotartSliderWithLabels(juce::RangedAudioParameter& rap, const juce::String& unitSuffix) : 
...
private:
    ...
    RotartSliderWithLabels peakFreqSlider,
        peakGainSlider,
        peakQualitySlider,
        lowCutFreqSlider,
        highCutFreqSlider,
